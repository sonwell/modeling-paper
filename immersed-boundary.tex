...
three pieces of information for each immersed structure: the points \hi{$\X_j$ used to
evaluate forces on the structure}; a force density $\F(\X_j)$ at each of those points;
...
\hi{$\X_j$}, $\F(\X_j)$, and $\weight[j]$ (Section~\ref{sec:rbfs}). However, before we
...
used in this work also does something similar. A small set of \hi{$\data\cardinality$
Lagrangian points} approximately $2h$ apart is used to approximate $\X$ and this set of
points is moved by~\eqref{eq:ib-interp}. In addition, a larger set of
\hi{$\sample\cardinality$ points} chosen to be approximately $h$ apart is used to spread
