...
$\poly\cardinality \ge (m+1)^2$ spherical harmonics, \latin{i.e.}, $P$ is of full rank~%
...
with the point $\Xp(\theta,\,\varphi)$ on $\sphere$. Consequently, by \hi{sampling $\X$
at each point in $\data\sites$}, we can approximately reconstruct the surface by
...
identity operator in order to obtain \hi{$\X$ at sample sites}. Evaluating $\L s$ at each
sample site we have
